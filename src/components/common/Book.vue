<template>
  <div>
    <mt-header
      ...
      <div
        slot="left"
        @click="$router.go(backStep)">
        <mt-button icon="back">返回</mt-button>
      </div>
      <router-link
        v-if="book"
        :to="/changeSource/+book._id"
        slot="right">
    ...
    <section>
      <div class="book-info">
        ...
        <div class="book-info-detail">
          <p
            class="book-title"
            v-if="book">{{ book.title }}</p>
          <p
            class="book-author"
            v-if="book">{{ book.author }}</p>
          <p
            class="reader-info"
            v-if="book">
          <span/>{{ book.updated | ago }} | {{ wordCount }}万 | {{ book.cat }}</p>
        </div>
      </div>
      <div class="book-operation">
        <mt-button
          type="primary"
          class="btn"
          @click="followAction">{{ isFollowed ? '不追了' : '追更新' }}</mt-button>
        <mt-button
          type="primary"
          class="btn"
          @click="readBook">开始阅读</mt-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    followAction () {
      let localShelf = util.getLocalStroageData('followBookList')
        ? util.getLocalStroageData('followBookList')
        : {}
      if (this.isFollowed) {
        // 删除该书籍在本地的缓存记录
