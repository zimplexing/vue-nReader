<template>
  <div
    ...
    <mt-header
      fixed
      :title="$store.state.bookInfo.title"
      v-if="operation"
      class="head">
      <router-link
        :to="/book/+$store.state.bookInfo._id"
        slot="left">
    ...
    <div
      ...
      <ul
        ...
        <v-touch
          ...
          :key="index"
          @tap="jumpChapter(index)">{{ chapter.title }}</v-touch>
      </ul>
    </div>
  </div>
</template>
